...
private:
    ...
    void read_beat(axi4_ref<A_WIDTH, D_WIDTH, ID_WIDTH> &pin)
    {
        ...
        // std::cout << " rready: " << (uint64_t)pin.rready;
        // std::cout << " cur trans: " << (uint64_t)r_cur_trans;
        // std::cout << " nr trans: " << (uint64_t)r_nr_trans << std::endl;
...
private:
    ...
    bool write_check()
    {
        if (w_burst_type == BURST_RESERVED || w_burst_type == BURST_FIXED)
            return false;
